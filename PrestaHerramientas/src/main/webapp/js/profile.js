    ...
    await $.ajax({
        ...
        success: function (result) {
            ...
            if (parsedResult != false) {
                ...
                $("#input-nombre").val(parsedResult.nombre_usu);
...
function mostrarHistorial(herramienta) {
    let contenido = "";
    if (herramienta.length >= 1) {
        $.each(herramienta, function (index, herramienta) {
            ...
            contenido += '<tr><th scope="row">' + herramienta.id + '</th>' +
                    ...
                    '<td><input type="checkbox" name="disponible" id="disponible' + herramienta.id 
                    + '" disabled ';
...
function modificarUsuario() {
    ...
    let frecuente = $("#input-frecuente").prop('checked');
    $.ajax({
        type: "GET",
        dataType: "html",
